...
export default function ShippingRates({ shippings }: ShippingRatesProps) {
   ...
   useEffect(() => {
      const defaultCountry =
         shippings?.find((s) => s.isDefault)?.countryName ||
         shippings?.[0]?.countryName ||
         "";
   ...
   const handleGetRates = () => {
      const filtered = shippings?.filter(
         (s) => s.countryName === selectedCountry
      );
   ...
   return (
      <section>
         <h2 className="mb-6 text-lg font-semibold text-foreground mt-6">
            Shipping rates
         </h2>
